...
int main()
{
	...
	const char *path1 = "ic.txt";   // Intermediate code file
	const char *path2 = "symtable.txt"; // Symbol table file
	const char *path3 = "littable.txt"; // Literal table file
	...
	while(!fin.eof())
	{
		...
		if(opcode == "ORIGIN") 
		{
			...
			if(plus != -1)
			{
				string base = op1.substr(0, plus);
				int id = getSymID(base);
				lc = ST[id].addr;
				lc = to_string(stoi(lc) + stoi(op1.substr(plus + 1)));
				
				// Adding to IC for "+" case
				IC += "(S,0" + to_string(ST[id].no) + ")+" + op1.substr(plus + 1) + " NAN";
			}
			else if(minus != -1)
			{
				string base = op1.substr(0, minus);
				int id = getSymID(base);
				lc = ST[id].addr;
				lc = to_string(stoi(lc) - stoi(op1.substr(minus + 1)));
				
				// Adding to IC for "-" case
				IC += "(S,0" + to_string(ST[id].no) + ")-" + op1.substr(minus + 1) + " NAN";
			}
			else
			{
				int id = getSymID(op1);
				...
				// Adding to IC when there’s no operator
				IC += "(S,0" + to_string(ST[id].no) + ") NAN NAN";
			}
